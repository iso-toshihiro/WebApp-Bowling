...
class ScoresController < ApplicationController
  ...
  def edit
    ...
    @down_pins = score_object.get_down_pin_list(@scores)
    @pin_list = score_object.get_down_pin_list(@scores)
    @total = score_object.get_score_list(@scores)
    @from = 'edit'
    @id = params[:id]
  end
  ...
  def confirm
    @from = params[:from]
    @name = params[:new_player_name]
    if params[:current_time] == 'on'
      @date = Time.now
    else
      @date = params[:new_game_date]
    end
    @down_pins = params[:new_down_pin]
    scores = ScoreCalculator.new (@down_pins)
    scores.calculate_scores
    @total = scores.scores
    if @from == 'edit'
      @id = params[:id]
      @old_name = params[:old_player_name]
      @old_date = params[:old_game_date]
      @old_down_pins = params[:old_down_pin]
  ...
  def update
    ...
    @score.update_attributes( :player_name => @name,
                  :game_date => @date,
                  :down_pins_1st_roll_1_frame => @down_pins[0],
                  :down_pins_2nd_roll_1_frame => @down_pins[1],
                  :down_pins_1st_roll_2_frame => @down_pins[2],
